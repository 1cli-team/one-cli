...
description: All 10 deploy backends compared. Mixed workspaces. Multi-environment trees. Per-project local Profile bindings and dry-run.
---
...
## Per-project local Profile binding

The Manifest records the deploy Backend and its repository-owned configuration, but never a local Profile name. To make this checkout use `prod-cluster` for the `api` Project in `prod`, run `one serve`, select **Production**, open `api`, and choose that Profile. The Dashboard writes only the name to `~/.config/one/profile-bindings.json`; it does not edit `one.manifest.json`.

This boundary is intentional: the repository makes the Backend and target environment reviewable, while each machine or CI runner maps them to its own credentials. Another checkout, even with the same Manifest Workspace ID, gets an independent binding because the local store is keyed by canonical root. CI can also use an explicit one-shot `--profile`.
