...
//
// Refresh fixtures with: UPDATE_SNAPSHOTS=1 go test ./tests/e2e/ -run TestHelpSnapshots
//
...
func helpSnapshotDir(t *testing.T) string {
	...
	// snapshot_help_test.go lives at packages/cli/tests/e2e/ — two dirs up gets
	// us to packages/cli/, which contains testdata/.
...
func TestRootHelpSnapshot(t *testing.T) {
	_ = i18n.Init(i18n.DefaultLocale)
	assertHelpSnapshot(t, "root.txt", cli.RootHelp())
}
