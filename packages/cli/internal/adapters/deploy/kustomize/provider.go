...
// interface. Apply orchestrates the kustomize-specific
...
type providerImpl struct {
	providerTag func() string
}

// NewProvider constructs the compiled kustomize adapter. Container pre-builds
// use the same Docker adapter as the container module; the build tag is
// command-scoped state.
func NewProvider(buildTag string) deploy.Provider {
	return providerImpl{
		providerTag: func() string {
			return buildTag
		},
...
func (p providerImpl) Apply(ctx context.Context, in deploy.ApplyInput) (*deploy.ApplyResult, error) {
	...
	if containerEnabledForProject(in.Manifest, in.Project.Name) {
		...
		buildTag := ""
		if p.providerTag != nil {
			buildTag = p.providerTag()
		}
		imageTag, err := selectDeployImageTag(in.Stdout, in.Manifest, in.Project.Name, buildTag)
		if err != nil {
			return nil, err
		}
		if err := p.buildAndPushContainer(ctx, in, imageTag, reg, platform); err != nil {
			return nil, err
		}
...
func (p providerImpl) buildAndPushContainer(ctx context.Context, in deploy.ApplyInput, tag string, reg *container.Registry, platform string) error {
	root := in.ProjectRoot
	dryRun := in.DryRun
	stdout := in.Stdout
	if stdout == nil {
	...
	buildRes, err := docker.Build(ctx, container.BuildInput{
		ProjectRoot: root,
	...
	if buildRes != nil {
		for _, e := range buildRes.Built {
			if dryRun {
				...
				if effectiveTag == "" {
					effectiveTag = container.ImageTagVersion(e.Image)
				}
				continue
			}
			...
			if err := workspace.SetProjectBuildVersion(root, e.Project, container.ImageTagVersion(e.Image)); err != nil {
				return err
			}
			if effectiveTag == "" {
				effectiveTag = container.ImageTagVersion(e.Image)
	...
	pushRes, err := docker.Push(ctx, container.PushInput{
		ProjectRoot: root,
		Project:     in.Project.Name,
		TargetNames: []string{in.Project.Name},
		Tag:         effectiveTag,
...
func currentImageSemverTag(m *workspace.Manifest, subprojectName string) (semverTag, bool) {
	...
	for _, sub := range m.Projects {
		...
		if tag, ok := parseSemverTag(container.ImageTagVersion(sub.Domains.Container.Image)); ok {
			return tag, true
		}
	}
