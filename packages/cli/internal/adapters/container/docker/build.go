...
// has credentials, then shells out to `docker build` via process.
...
import (
	...
	"github.com/torchstellar-team/one-cli/packages/cli/internal/platform/process"
)
...
func Build(ctx context.Context, in container.BuildInput) (*container.BuildResult, error) {
	...
	for _, s := range m.Projects {
		...
		if err := process.RunExternal(in.ProjectRoot, argv, "请安装 Docker Desktop / Engine"); err != nil {
			return nil, err
		}
		res.Built = append(res.Built, entry)
	}
