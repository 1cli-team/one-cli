	...
	for _, input := range specs {
		...
		for _, field := range spec.Profile.Fields {
			if strings.TrimSpace(field.Path) == "" || strings.TrimSpace(field.InputName) == "" || strings.TrimSpace(field.LabelKey) == "" {
				return nil, fmt.Errorf("catalog: backend %q has incomplete field metadata", spec.Pair)
			}
			if _, exists := paths[field.Path]; exists {
				return nil, fmt.Errorf("catalog: backend %q declares field path %q twice", spec.Pair, field.Path)
			}
			if _, exists := inputs[field.InputName]; exists {
				return nil, fmt.Errorf("catalog: backend %q declares input %q twice", spec.Pair, field.InputName)
			}
			...
			case FieldString:
				if field.Default != nil {
					if _, ok := field.Default.(string); !ok {
						return nil, fmt.Errorf("catalog: backend %q field %q has a non-string default", spec.Pair, field.Path)
					}
				}
			case FieldSecret:
				if field.Default != nil {
					return nil, fmt.Errorf("catalog: backend %q secret field %q declares a default", spec.Pair, field.Path)
				}
			case FieldBoolean:
				if field.Default != nil {
					if _, ok := field.Default.(bool); !ok {
						return nil, fmt.Errorf("catalog: backend %q field %q has a non-boolean default", spec.Pair, field.Path)
					}
				}
			default:
				return nil, fmt.Errorf("catalog: backend %q field %q has unknown type %q", spec.Pair, field.Path, field.Type)
			}
		}
