...
func (c *Config) S3CompatSection(kind string) *Section[S3Profile] {
	switch kind {
	case catalog.DeployAliyunOSS:
		return &c.DeployAliyunOSS
	case catalog.DeployTencentCOS:
		return &c.DeployTencentCOS
	case catalog.DeployAWSS3:
		return &c.DeployAWSS3
	case catalog.DeployMinIO:
		return &c.DeployMinIO
	case catalog.DeployRustFS:
		return &c.DeployRustFS
	case catalog.DeployR2:
		return &c.DeployR2
	}
	return nil
}
...
// backend ids in Backend Catalog order. Listed once here so callers can range
// over the same protocol family without re-typing the literals.
func S3CompatKinds() []string {
	specs := catalog.Builtin().WithTrait(catalog.TraitS3Compatible)
...
func IsS3Compatible(backend string) bool {
	spec, ok := catalog.Builtin().Lookup(catalog.DomainDeploy, backend)
	return ok && spec.HasTrait(catalog.TraitS3Compatible)
}

// ContainerKindSection returns the profile-section for one container
// backend kind. All four kinds share Section[ContainerProfile]; the schema
// policy table uses this one typed storage dispatcher for the family.
func (c *Config) ContainerKindSection(kind string) *Section[ContainerProfile] {
	switch kind {
	case catalog.ContainerDocker:
		return &c.ContainerDocker
	case catalog.ContainerDockerHub:
		return &c.ContainerDockerHub
	case catalog.ContainerGHCR:
		return &c.ContainerGHCR
	case catalog.ContainerACR:
		return &c.ContainerACR
	}
	return nil
}

// ContainerKinds returns container backend ids in Backend Catalog order.
func ContainerKinds() []string {
	return catalog.Builtin().Names(catalog.DomainContainer)
}
...
func IsContainerKind(backend string) bool {
	_, ok := catalog.Builtin().Lookup(catalog.DomainContainer, backend)
	return ok
}
...
// S3-compatible deploy backend. Sibling of Config.S3CompatSection and the
// single typed credentials-storage dispatcher used by the policy table.
...
const (
	...
	DomainDeploy    = catalog.DomainDeploy
	DomainContainer = catalog.DomainContainer
)
