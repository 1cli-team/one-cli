...
// is Vercel. ProjectID is the `prj_xxx` id Vercel assigns; ProjectName is
// its display slug. Env names the deploy target environment (drawn from
// manifest.environments.names; empty / "prod" → production deploy, anything
// else → preview).
...
// DecodeProjectConfig pulls the Vercel-specific config blob out of the
// manifest's per-project deploy section. Returns (nil, nil) when no
// project with that name has a Vercel deploy section configured.
func DecodeProjectConfig(m *workspace.Manifest, projectName string) (*ProjectConfig, error) {
	...
	for _, p := range m.Projects {
		...
		if p.Domains == nil || p.Domains.Deploy == nil || p.Domains.Deploy.Kind != workspace.DeployBackendVercel {
			return nil, nil
		}
...
func EncodeProjectConfig(p *workspace.ManifestProject, cfg *ProjectConfig) error {
	...
	if p.Domains.Deploy == nil {
		p.Domains.Deploy = &workspace.ProjectDeployBackend{Kind: workspace.DeployBackendVercel}
	}
	if cfg == nil {
