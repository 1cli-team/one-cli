...
func containerSubprojects(manifest *workspace.Manifest) ([]string, error) {
	var names []string
	anyEnabled := false
	for _, project := range manifest.Projects {
		enabled, _ := workspace.ContainerForProject(manifest, project.Name)
		if enabled {
			anyEnabled = true
...
// (meaning "all").
func normalizeContainerProject(activeWorkspace execution.Workspace, selector string) string {
