...
// chain).
func resolveBuildTag(manifest *workspace.Manifest, subproject string, targetNames []string, explicitTag string) (string, error) {
	...
	return selectBuildImageTag(manifest, promptSubproject)
}
...
func currentImageSemverTag(m *workspace.Manifest, subprojectName string) (semverTag, bool) {
	...
	for _, sub := range m.Projects {
		...
		if tag, ok := parseSemverTag(containermodel.ImageTagVersion(sub.Domains.Container.Image)); ok {
			return tag, true
		}
	}
