...
func testServices(t *testing.T) (*catalog.Catalog, *configureapp.ProfileService, *workspaceapp.Service) {
	t.Helper()
	backendCatalog := catalog.Builtin()
	profiles, err := configureapp.NewProfileService(backendCatalog, configureapp.LocalProfileRepository{})
	if err != nil {
		t.Fatal(err)
	}
	workspaces, err := workspaceapp.NewService(backendCatalog)
	if err != nil {
		t.Fatal(err)
	}
	return backendCatalog, profiles, workspaces
}
...
func TestBuildContributionsTreeShape(t *testing.T) {
	backendCatalog, profiles, workspaces := testServices(t)
	cmds := buildContributions(backendCatalog, profiles, workspaces)
	if len(cmds) != 1 {
...
func TestAddHelpExamplesAreBackendSpecific(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(strings.Join(tt.path, "/"), func(t *testing.T) {
			backendCatalog, profiles, workspaces := testServices(t)
			parent := buildContributions(backendCatalog, profiles, workspaces)[0]
...
func TestConfigureWizardDispatchClearsOriginalArgs(t *testing.T) {
	...
	err := runSelectedAddBackend(backendCatalog, profiles, parent, profile.DomainEnv, "infisical")
	if err == nil {
