...
type LoadInjectionOptions struct {
	// Loaders is the command-tree's instance-scoped loader registry.
...
func LoadInjectionEnv(ctx context.Context, in ApplyInput, opts LoadInjectionOptions) (*InjectionResult, error) {
	...
	if id := strings.TrimSpace(opts.LoaderID); id != "" {
		loader = opts.Loaders.Find(id)
		if loader == nil {
			all := opts.Loaders.All()
		...
		loader = opts.Loaders.PickAvailable(in.ProjectRoot)
	}
	if loader == nil {
		return nil, nil
	}
