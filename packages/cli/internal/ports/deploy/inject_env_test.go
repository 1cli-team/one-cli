...
func loaderOptions(loaders ...secrets.Loader) LoadInjectionOptions {
	return LoadInjectionOptions{Loaders: secrets.MustRegistry(loaders...)}
}
...
func TestLoadInjectionEnv_HappyPath(t *testing.T) {
	...
	res, err := LoadInjectionEnv(context.Background(), in, opts)
	if err != nil {
...
func TestLoadInjectionEnv_PerProjectDisabled(t *testing.T) {
	opts := loaderOptions(&stubLoader{id: "stub", priority: secrets.PriorityFilesystem, available: true, vars: map[string]string{"X": "y"}})
...
func TestLoadInjectionEnv_EmptyVarsDowngrades(t *testing.T) {
	...
	opts := loaderOptions(&stubLoader{id: "dotenv", priority: secrets.PriorityFilesystem, available: true, vars: map[string]string{}})
...
func TestLoadInjectionEnv_OtherErrorBubbles(t *testing.T) {
	...
	opts := loaderOptions(&stubLoader{id: "infisical", priority: secrets.PriorityRemoteBackend, available: true, err: netErr})

	root := t.TempDir()
	writeManifest(t, root, "web", false)
	in := baseInput(t, root, "web")

	_, err := LoadInjectionEnv(context.Background(), in, opts)
	if err == nil {
...
func TestLoadInjectionEnv_UnknownLoaderID(t *testing.T) {
	opts := loaderOptions(&stubLoader{id: "dotenv", priority: secrets.PriorityFilesystem, available: true})
	opts.LoaderID = "does-not-exist"
...
func TestLoadInjectionEnv_EmptyVarsReturnsNil(t *testing.T) {
	opts := loaderOptions(&stubLoader{id: "stub", priority: secrets.PriorityFilesystem, available: true, vars: map[string]string{}})
