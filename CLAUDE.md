...
> git describe --tags --always  # repository/release identity
> ```
>
...
`packages/cli/internal/resources/bundled/` is `go:embed`-ed at build time and
...
`packages/cli/internal/resources/bundled/skills/`. It runs as a dep of `vet` /
...
- snapshot tests under `packages/cli/tests/e2e/` lock the JSON envelope shape
- `install_sh_test.go` keeps the curl installer's safety sentinels in place

The whole `packages/cli/internal/resources/bundled/` tree is gitignored: don't
...
- **Command names**: composition root (`packages/cli/internal/bootstrap/cli/`)
  and Cobra transports (`packages/cli/internal/transport/cobra/*/`). Verified against every prose doc by
...
- **Help text**: every cobra command's `--help` output is snapshotted
  ...
  in `packages/cli/internal/bootstrap/cli/root.go` must list exactly the
  ...
  `UPDATE_SNAPSHOTS=1 go test ./tests/e2e/ -run TestHelpSnapshots`.
- **Error codes**: `packages/cli/internal/platform/errors/codes.go`. Generated
  to `apps/docs/content/docs/reference/error-codes.md` by
  `task gen-error-codes`.
- **Release version**: the `Build and Release` workflow computes it from the
  selected semantic-version bump and the highest stable tag. It creates the
  matching `vMAJOR.MINOR.PATCH` tag, and GoReleaser injects that tag version
  through ldflags. Skill metadata versions and documentation examples are
  independent content versions, not release state.
...
- Don't paste current state into this file. Repo overview, command
  ...
  (`task --list`, `git ls-files`, `git describe --tags --always`). This file is for
